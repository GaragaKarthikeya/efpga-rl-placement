...
island-style baseline architecture. The Area-Delay-Power Product (ADP) acts as a comprehensive PPA metric, evaluating the design's trade-offs by combining all three physical constraints into a single, joint optimization figure of merit, as expressed in Eq.~(\ref{eq:adp}).

\subsection{RL Formulation}
\label{sec:rl}

We pose placement as a finite-horizon sequential decision process and train
the policy using MaskablePPO~\cite{maskableppo}, an action-masked variant of
the PPO reinforcement learning algorithm, which restricts the action space to
legal tile coordinates at each step.
...
the environment renders the layout into an architecture XML via Jinja2~\cite{jinja2} and invokes
...
regardless of which benchmark circuit is active. Together, the step-0 aspect
ratio and the subsequent tile coordinates instantiate the policy output
$\pi$ defined in Eq.~(\ref{eq:policy}). Action masking then
...
penalty of $-10$ in place of the reward computed by Eq.~(\ref{eq:reward}).
...
avoiding overfitting to any single benchmark circuit's topology. We use
MaskablePPO, training each run for 13,000 episodes. We report three
independent seeds (7, 42, 123) throughout Section~\ref{sec:results} to
characterize reliability rather than present a single run as ground truth.
...
Koios~\cite{koios}, the wider VTR codebase~\cite{vtr9}, and two small probes authored for this work, covering the
...
GOLDS over H-block coordinates and aspect ratio. The GA and the policy search in
...
\midrule
\texttt{macbuf}           & 65.33 & 728 & 55.46 \\
\texttt{mkDelayW32B}      & 17.92 & 746 & 58.73 \\
\texttt{cipher}           & 44.13 & 576 & 33.49 \\
\texttt{reduction\_layer} & 7.18 & 731 & 23.30 \\
\texttt{arm\_core}        & 17.88 & 736 & 9.11 \\
\texttt{softmax}          & 15.61 & 784 & 7.27 \\
\bottomrule
\end{tabular}%
}
\end{table}
...
at~\cite{efpga_rl_artifact_2027} for further use by the research and design
community.
